...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["13132900007@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["13132900007@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["13132900007@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			case 'owner':
				...
				case 'fake':          
                    ...
                    break
                case 'fakereplay':
                   client.reply(from, 'ange mas', 'mending lari', "1226-4440009")
                   break
                case 'but':
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    client.sendMessage('13132900007@s.whatsapp.net', options, text, {quoted: mek})
               ...
               case 'infogempa':
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 D1CX BOT 〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa?')
                ...
                case 'bc3':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *D1CXBOT Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner D1CXBOT!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin D1CXBOT!')
				   }
                   break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
